...
req.prototype.option = function (name, value) {
    setter(name, value, this.options);
    return this;
};
req.prototype.cookie = function (name, value) {
    setter(name, value, this.cookies);
    return this;
};
req.prototype.header = function (name, value) {
    setter(name, value, this.headers);
    return this;
};
/** adds form post variables */
req.prototype.postVar = function (name, value) {
...
req.prototype.getTransmissionOptions = function () {
    var ops = this.options, headers = this.headers;
    /* headers */
    ...
    var cookieString = index_1.ackX.object(cookies).toCookieString();
...
req.prototype.send = function (address, options) {
    var req, ops = this.getTransmissionOptions(), body = '';
    if (address) {
        var orgUrl = ops.uri;
    ...
    this.preSendOps(ops);
    var promise = index_1.ackX.promise().bind(this)
        .callback(function (callback) {
        req = request[ops.method.toLowerCase()](ops, callback);
    });
    var spread = ops.spread == null ? this.options.spread : ops.spread;
    if (spread == null || spread) {
        ...
        promise = promise.then(function (response, body) { return response; });
    }
    return promise;
};
...
function method(urlOrScope) {
    return new req(urlOrScope);
}
exports.method = method;
function setter(name, value, data) {
