...
ackTask.prototype.getLogFilePath = function () {
    ...
    this.data.logFilePath = index_js_1.ackX.path(__dirname).join('logs', 'ackTaskLog.txt').path;
    return this.data.logFilePath;
};
ackTask.prototype.getLogFile = function () {
    return index_js_1.ackX.file(this.getLogFilePath());
};
ackTask.prototype.getRequirePromiseByPath = function (path) {
    var promise = index_js_1.ackX.promise().callback(function (callback) {
        var exec = require('child_process').exec, cmdProc = function (error, stdout, stderr) {
            if (error) {
                error.name = 'node-cmd';
